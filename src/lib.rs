...
pub fn parse_stdin<E: Fn(ParseIntError) -> io::Error, F: Fn(String)>(
    ...
    loop {
        ...
        if nbytes == 0 || (break_nl && (line == NEWLINE)) {
            return Ok(());
        }
...
pub fn hex2dec_line<S: AsRef<str>>(
    ...
    let new_str = replace_all(&REGEX, line.as_ref(), |caps: &Captures| {
        let m = caps.get(0).unwrap();
        ...
        Ok(format!("{:width$}", dec, width = format_length))
    })?
    .into_owned();
