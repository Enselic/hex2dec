...
pub fn parse_stdin<F: Fn(String)>(f: F, break_nl: Option<bool>) -> Result<(),std::io::Error>{
    let mut line = String::new();
    let mut handle = std::io::stdin().lock();

    let break_newline = break_nl.unwrap_or(false);
    loop {
        let nbytes = handle.read_line(&mut line)?;

        // Break if EOF
        // or a blank line is reached with break_newline true.
        if nbytes == 0 || (break_newline && (line == NEWLINE)) { return Ok(()); }
        ...
        match hex2dec_line(&line) {
            ...
            Err(e) => return Err(
                std::io::Error::new(
                    ...
                    format!("ParseIntError: failed to parse hex value; {}", e.to_string())
                )
            )
        };
...
/// Wrapper for [`parse_stdin`] for use with CI utilities.
/// Function attempts to parse entire file with errors redirected to STDOUT.
/// ### Note
/// This function will stop parsing if an error is encountered.
pub fn parse_stdin_ci() {
    if let Err(e) = parse_stdin(|s| println!("{}", s), Some(false)) {
        println!("{}", e)
    }
}
